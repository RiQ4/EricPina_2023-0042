...
void impresion(int M[][TAM], int F); //Declara los prototipos de las funciones impresion y suma de matrices.
void suma(int M1[][TAM], int M2[][TAM], int F);

void main(void) //Funcion main() donde se declaran los arreglos a ser operados. Se llama la funcion suma.
{
    int A1[TAM][TAM] = {{3, 5, 7},  //Valores del Arreglo 1.
                        {9, 11, 13},
                        {15, 17, 19}};
    ...
    int A2[TAM][TAM] = {{2, 4, 6}, //Valores del Arreglo 2.
                        {8, 10, 12},
                        {14, 16, 18}};
    suma(A1, A2, TAM);                  //Llamada de la funcion suma, pasando A1, A2 y su tamano.
...
void suma(int M1[][TAM], int M2[][TAM], int F) //Funcion para sumar dos arreglos 2D.
{
    ...
    for (int i = 0; i<F; i++) //Itera por las filas.
    {
        ...
        {
            R[i][j] = M1[i][j] + M2[i][j]; //Asigna valores a R mediante la suma de M1 y M2
        }
    }
